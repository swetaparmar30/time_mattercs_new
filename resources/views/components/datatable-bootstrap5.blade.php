...
@php
    ...
    function generateFormFields($method = 'POST') {
        $csrf = csrf_token();
        $methodField = $method !== 'POST' ? '<input type="hidden" name="_method" value="' . strtoupper($method) . '">' : '';
...
<!-- <div class="table-responsive">
    <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns"> -->
        ...
        <div class="dt-responsive {{ $responsive ? 'table-responsive' : '' }}">
            <table class="table table-striped table-bordered nowrap" id="{{ $tableId }}">
                <thead>
                    <tr>
                        @foreach($columns as $column)
                                <!-- data-sortable="{{ $sortable && ($column['sortable'] ?? false) ? 'true' : 'false' }}" 
                                data-sort-key="{{ $column['key'] ?? '' }}" -->
                            <th 
                                style="width: {{ $column['width'] ?? 'auto' }};"
                            >
                                <!-- @if($sortable && ($column['sortable'] ?? false))
                                    <button class="datatable-sorter">{{ $column['title'] ?? $column['key'] }}</button>
                                @else -->
                                        ...
                                        @elseif(isset($column['key']))
                                            ...
                                            @if(isset($column['type']))
                                                @switch($column['type'])
                                                    @case('image')
                                                        @if($value)
                                                            <img src="{{ $value }}" alt="{{ $column['alt'] ?? 'Image' }}" 
                                                    ...
                                                    @case('avatar')
                                                        <div class="d-flex align-items-center">
                                                            <div class="flex-shrink-0">
                                                                @if($value)
                                                                    <img src="{{ $value }}" alt="user image" class="img-radius wid-40">
                                                                @else
                                                                    <div class="avatar wid-40 bg-light">
                                                                        <i class="ph ph-user"></i>
                                                                    </div>
                                                            ...
                                                            <div class="flex-grow-1 ms-3">
                                                                <h6 class="mb-0">{{ data_get($row, $column['nameKey'] ?? 'name') }}</h6>
                                                            </div>
                                                        </div>
                                                        @break
                                                    ...
                                                    @case('status')
                                                        @php
                                                            ...
                                                            $statusIcon = $value ? 'ti ti-circle-filled' : 'ti ti-circle-filled';
                                                        @endphp
                                                        <span class="{{ $statusClass }}">
                                                            <i class="{{ $statusIcon }} align-baseline f-10 m-r-10"></i> {{ $statusText }}
                                                        </span>
                                                        @break
                                                    ...
                                                    @case('date')
                                                        ...
                                                        @endphp
                                                        {{ \Carbon\Carbon::parse($value)->format($dateFormat) }}
                                                        @if($timeFormat)
                                                            <span class="text-muted text-sm d-block">
                                                                {{ \Carbon\Carbon::parse($value)->format($timeFormat) }}
                                                            </span>
                                                        @endif
                                                        @break
                                            ...
                                            @foreach($actions as $action)
                                                ...
                                                @elseif($action['type'] === 'delete')
                                                    <form action="{{ $action['url']($row) }}" method="POST" 
                                                          ...
                                                          id="deleteForm{{ $row->id ?? $index }}">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                    <button type="button" 
                                                            class="avatar avatar-xs mx-1 btn btn-link-secondary delete-btn"
                                                            data-form-id="deleteForm{{ $row->id ?? $index }}"
                        ...
                        <tr>
                            <td colspan="{{ count($columns) + ($hasActions ? 1 : 0) }}" class="text-center py-5">
                                <div class="d-flex flex-column align-items-center">
                                    <i class="{{ $emptyIcon }}" style="font-size: 48px; color: #ccc; margin-bottom: 1rem;"></i>
...
@if($hasData)
    ...
    <script src="{{ asset('assets/js/plugins/dataTables.bootstrap5.min.js') }}"></script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const searchInput = document.querySelector(`#${tableId}`).closest(`#${tableId}_wrapper`).querySelector(`#${tableId}_input`);
            const perPageSelect = document.querySelector(`#${tableId}`).closest(`#${tableId}_wrapper`).querySelector(`#${tableId}_selector`);
            const sortButtons = document.querySelectorAll(`#${tableId} #${tableId}_sorter`);
            ...
            let currentSortDirection = new URLSearchParams(window.location.search).get('sort_direction') || null;
            
            // Helper function to preserve all current parameters
            function preserveCurrentParameters(url) {
                // Preserve current sort parameters
                if (currentSortBy && currentSortDirection) {
                    url.searchParams.set('sort_by', currentSortBy);
                    url.searchParams.set('sort_direction', currentSortDirection);
                }
                ...
                const currentPerPage = perPageSelect ? perPageSelect.value : '';
                if (currentPerPage) {
            ...
            if (searchInput) {
                ...
                const ajaxSearch = debounce(function() {
                    ...
                    // Preserve all other current parameters
                    preserveCurrentParameters(url);
                    ...
                    .then(html => {
                        ...
                        const newTable = doc.querySelector(`#${tableId}`).closest(`#${tableId}_container`);
                        const currentTable = document.querySelector(`#${tableId}`).closest(`#${tableId}_container`);
                        ...
                        const currentPagination = document.querySelector('.d-flex.justify-content-center.mt-3');
            ...
            document.addEventListener('click', function(e) {
                if (e.target.closest(`#${tableId}_sorter`)) {
                    e.preventDefault();
                    const button = e.target.closest(`#${tableId}_sorter`);
            ...
            function updateSortIndicators() {
                ...
                const currentSortButtons = document.querySelectorAll(`#${tableId} #${tableId}_sorter`);
